...
<style>
    ...
    /* Select Box - Consistent Light Background */
    .stSelectbox > div > div {
        border-radius: 8px !important;
        border: 1px solid #E5E7EB !important;
...
def complete_visit(visit_id, diagnosis, prescription, patient_phone=None):
    try:
        ...
        #         patient_phone,
        #         SMSTemplates.consultation_complete(token_str)
        #     )
            ...
            if 'current_patient' in st.session_state and st.session_state.current_patient:
                ...
                with col_complete:
                    if st.button("✅ Complete Visit", type="primary", use_container_width=True):
                        if diagnosis:
                            if complete_visit(p['id'], diagnosis, prescription, p.get('phone')):
